...
function update_play_state_display(msg) {
  if(msg.tempo) {
    ...
    document.getElementById(`ch${msg.ch}_${player_header_items[TIMBRE_INDEX]}`).innerHTML = msg.timbre;
  } else if(msg.vel) {
    document.getElementById(`ch${msg.ch}_${player_header_items[NOTE_INDEX]}`).innerHTML = `OFF ${msg.key}`
  } else {
    document.getElementById(`ch${msg.ch}_${player_header_items[NOTE_INDEX]}`).innerHTML = `ON ${msg.key}`
  }
}